...
function EditModuleTemplate(props) {
  let temp = useRef(0);
  ...
  async function selectingArguments() {
    let flag = 0;
    const valueBody = [];
    ...
    if (flag == 0) {
      ...
      const response = await AxiosInstance.post("/editmodule", {
        provider: provid.toLowerCase(),
        module_name: template_name,
        module_desc: template_desc,
        data: valueBody,
  ...
  async function editingTemplate(e) {
    const response = await AxiosInstance.post("/getmoduletemplate", {
      module_name: e,
    });
    ...
    for (const a of data[0].data) {
      ...
      for (const b of data1) {
        if (e == b.module_name) {
          dispatch(updateTemplateName(b.module_name));
...
export default React.memo(EditModuleTemplate);
